...
parser.add_argument('-svfg','--savefig', metavar='SAVEFIG_DIRECTORY', nargs='?', const=os.path.join(os.getcwd(), 'output'),
                    help="Save the fraction plots generated by the program. Argument specifies the directory in which to save the figures. If no argument is given the figures are saved to './output/'.")

parser.add_argument('-svdr', '--savedir', metavar='SAVE_DIRECTORY', default=os.path.join(os.getcwd(), 'output'),
                    help="Specifies the directory in which to save the .dat file with residuals and their errors. Default is './output/'.")

parser.add_argument('-fit', action='store_true',
                    help='Fits the residuals of each channel with a constant and prints the chi squared and best fit constant to command line.')

parser.add_argument('-fig','--figure', action='store_true',
                    help='Show plots comparing the fractions of file 1 (blue) and file 2 (red) as a function of Q^2. The figures also show the residuals as a function of Q^2. If a fit was performed, the figure also shows the best fit constant.')
...
# Error control external to the frextract() function because it depends on the comparison between fracts1 and fracts2, which are not simultaneously known by frextract.
...
if args.figure or args.savefig is not None:
    ...
    for i in range(4):
        ...
        plt.ylabel('Fraction')
        ax = plt.gca()
        ax.xaxis.set_tick_params(labelbottom=False) # Removes axis labels from top subplot for readability
